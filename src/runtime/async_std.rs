...
pub mod task {
    ...
    ///
    /// async-std aborts on panic, so panics are not surfaced through this error.
    #[derive(Debug)]
    pub enum JoinError {
        Cancelled,
    ...
    impl<T> Future for JoinHandle<T> {
        ...
        fn poll(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
            ::async_std::task::JoinHandle::poll(Pin::new(&mut self.0), cx).map(Ok)
        }
    }
    ...
    where
        F: Future<Output = T>,
    {
        Ok(::async_std::future::timeout(duration, f).await?)
    }
}
